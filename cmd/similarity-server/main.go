...
func main() {
	var (
		...
		chromaHost = flag.String("chroma-host", "localhost", "ChromaDB host")
		chromaPort = flag.Int("chroma-port", 8037, "ChromaDB port")
...
func (s *Server) setupRoutes() {
	http.HandleFunc("/similarity", s.enableCORS(s.handleSimilarity))
	http.HandleFunc("/health", s.enableCORS(s.handleHealth))
}
